...
import { GET } from '../../../../../src/pages/api/user-email/index';
import { UserEmailService } from '../../../../../src/services/user-email-service';
vi.mock('../../../../../src/services/user-email-service');
vi.mock('@/lib/api/api-guard');

describe('UserEmail API - GET ../../../../../src/pages/api/user-email/index', () => {
  beforeEach(() => {
    vi.resetAllMocks();
    vi.mocked(ApiGuard.protect).mockResolvedValue(undefined);
  });

  it('should call UserEmailService and return success', async () => {
    ...
    const serviceMethod =
      'GET'.toLowerCase() === 'post'
        ? 'create'
        : 'GET'.toLowerCase() === 'get'
          ? '../../../../../src/pages/api/user-email/index'.includes('[id]')
            ? 'get'
            : 'list'
          : 'GET'.toLowerCase() === 'put'
            ? 'update'
    ...
    vi.mocked((UserEmailService as Record<string, unknown>)[serviceMethod]).mockResolvedValue({
      success: true,
      data: { id: 'test-id' },
