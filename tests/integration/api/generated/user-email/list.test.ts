...
describe('UserEmail API - List', () => {
  ...
  describe('GET /api/user-email', () => {
    ...
    it('should allow member to list userEmails', async () => {
      // eslint-disable-next-line @typescript-eslint/no-unused-vars
      const actor = await client.as('user', {});

      // Cleanup first to ensure clean state
      await Factory.prisma.userEmail.deleteMany();

      // Seed data
      const _listSuffix = Date.now();
      await Factory.create('userEmail', { ...baseData });
      await Factory.create('userEmail', { ...baseData });

      const res = await client.get('/api/user-email');
    ...
    it('should verify pagination metadata', async () => {
      ...
      for (let i = 0; i < toCreate; i++) {
        const rec = await Factory.create('userEmail', { ...baseData });
        createdIds.push(rec.id);
      }

      // Page 1
      const res1 = await client.get('/api/user-email?take=5&skip=0');
      expect(res1.status).toBe(200);
      expect(res1.body.data.length).toBe(5);
      expect(res1.body.meta.total).toBe(15);

      // Page 2
      const res2 = await client.get('/api/user-email?take=5&skip=5');
    ...
    it('should filter by name', async () => {
      ...
      await Factory.create('userEmail', { ...data1 });
      await Factory.create('userEmail', { ...data2 });

      const res = await client.get('/api/user-email?name=' + val1);
      expect(res.status).toBe(200);
